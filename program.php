...
try {
    ...
    if (!$program) {
        header("Location: programs.php?lang=$locale");
        exit;
    }
    
    // Get related programs (2 random programs that are not the current one)
    $stmt = $pdo->prepare("SELECT * FROM programs WHERE slug != ? ORDER BY RAND() LIMIT 2");
    $stmt->execute([$slug]);
    $related_programs = $stmt->fetchAll(PDO::FETCH_ASSOC);

} catch (PDOException $e) {
    // Database connection failed
    // We'll set a flag to display an error message
    $dbError = true;
}
...
$translations = [
    ...
    'ko' => [
        ...
        'donate' => '후원하기',
        'get_involved' => '참여하기',
        'back_to_programs' => '프로그램 목록으로 돌아가기',
        'other_programs' => '다른 프로그램',
        'related_programs' => '관련 프로그램',
        'learn_more' => '자세히 보기',
        'copyright' => '© 2025 코스모 재단. 모든 권리 보유.',
        'db_error' => '데이터베이스 연결 오류. 나중에 다시 시도해 주세요.'
    ]
];
...
if (isset($dbError) && $dbError) {
    ...
    echo "<span class='block sm:inline'> " . $t['db_error'] . "</span>";
    echo "</div>";
    exit;
}
...
<head>
    ...
    <title><?php echo $isKorean ? $program['ko_title'] : $program['title']; ?> | <?php echo $t['site_title']; ?></title>
...
<body class="bg-gray-50">
    ...
    <header class="bg-white shadow-md">
        <div class="container mx-auto px-4 py-4 flex flex-col md:flex-row justify-between items-center">
            ...
            <div class="mb-4 md:mb-0 md:absolute md:right-8 md:top-4">
                <a href="?slug=<?php echo $slug; ?>&lang=en" class="<?php echo $locale === 'en' ? 'font-bold text-primary' : 'text-gray-600'; ?> mr-2">EN</a> | 
                <a href="?slug=<?php echo $slug; ?>&lang=ko" class="<?php echo $locale === 'ko' ? 'font-bold text-primary' : 'text-gray-600'; ?> ml-2">KO</a>
            </div>
    ...
    <section class="relative">
        <div class="h-64 md:h-96 bg-gray-800 bg-opacity-50 bg-cover bg-center" style="background-image: url('<?php echo $program['image']; ?>');">
            <div class="absolute inset-0 bg-black bg-opacity-50"></div>
            <div class="container mx-auto px-4 h-full flex flex-col justify-center relative z-10">
                <h1 class="text-3xl md:text-5xl font-bold text-white mb-4"><?php echo $isKorean ? $program['ko_title'] : $program['title']; ?></h1>
                <p class="text-lg md:text-xl text-white mb-8 max-w-2xl"><?php echo $isKorean ? $program['ko_description'] : $program['description']; ?></p>
            </div>
        </div>
    </section>
        ...
        <div class="container mx-auto px-4">
            <div class="max-w-4xl mx-auto">
                <div class="prose lg:prose-lg mx-auto">
                    <?php echo $isKorean ? $program['ko_content'] : $program['content']; ?>
                </div>
                
                <div class="mt-12 text-center">
                    <a href="donate.php?lang=<?php echo $locale; ?>" class="inline-block bg-accent hover:bg-opacity-90 text-white px-6 py-3 rounded-md font-semibold">
                        <?php echo $t['get_involved']; ?>
                    </a>
                </div>
                
                <div class="mt-12">
                    <a href="programs.php?lang=<?php echo $locale; ?>" class="inline-flex items-center text-primary hover:underline">
                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5 mr-2" viewBox="0 0 20 20" fill="currentColor">
                            <path fill-rule="evenodd" d="M9.707 16.707a1 1 0 01-1.414 0l-6-6a1 1 0 010-1.414l6-6a1 1 0 011.414 1.414L5.414 9H17a1 1 0 110 2H5.414l4.293 4.293a1 1 0 010 1.414z" clip-rule="evenodd" />
                        </svg>
                        <?php echo $t['back_to_programs']; ?>
                    </a>
                </div>
            </div>
        </div>
    ...
    <section class="py-16 bg-gray-50">
        <div class="container mx-auto px-4">
            <h2 class="text-3xl font-bold text-center mb-12"><?php echo $t['related_programs']; ?></h2>
            
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl mx-auto">
                <?php foreach($related_programs as $related_program): ?>
                <!-- Related Program Card -->
                <div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-lg transition-shadow">
                    <img src="<?php echo $related_program['image']; ?>" alt="<?php echo $isKorean ? $related_program['ko_title'] : $related_program['title']; ?>" class="w-full h-48 object-cover">
                    <div class="p-6">
                        <h3 class="text-xl font-semibold mb-2"><?php echo $isKorean ? $related_program['ko_title'] : $related_program['title']; ?></h3>
                        <p class="text-gray-600 mb-4"><?php echo $isKorean ? $related_program['ko_description'] : $related_program['description']; ?></p>
                        <a href="program.php?slug=<?php echo $related_program['slug']; ?>&lang=<?php echo $locale; ?>" class="text-primary font-medium hover:underline"><?php echo $t['learn_more']; ?></a>
                    </div>
                </div>
                <?php endforeach; ?>
            </div>
